// Task Management Page JavaScript (Simplified)
...
// Get enabled markets for a task
function getEnabledMarkets(itemId) {
    const mapping = taskMapping.find(m => m.Item_ID === itemId);
    if (!mapping) return [];
    
    return availableMarkets.filter(market => mapping[market] === 'x');
}
...
function renderTasksTable() {
    ...
    pageData.forEach((task, index) => {
        ...
        // Get enabled markets for this task
        const enabledMarkets = getEnabledMarkets(task.Item_ID);
        const marketDisplay = enabledMarkets.length > 0 
            ? `<span class="market-badges">${enabledMarkets.map(m => 
                `<span class="market-badge">${m}</span>`).join(' ')}</span>`
            : '<span class="no-markets">None</span>';
...
function exportTasks() {
    // Create export data with market information
    const exportData = filteredTasks.map(task => {
        const mapping = taskMapping.find(m => m.Item_ID === task.Item_ID);
        const taskExport = { ...task };
        
        // Add market columns
        availableMarkets.forEach(market => {
            taskExport[market] = (mapping && mapping[market] === 'x') ? 'Yes' : 'No';
        });
