...
async function fetchCSVFromConfluence(filename) {
  try {
    const attachmentResponse = await fetch(
      `${CONFLUENCE_BASE_URL}/rest/api/content/${CONFLUENCE_PAGE_ID}/child/attachment?filename=${filename}`
    );
    
    const attachmentData = await attachmentResponse.json();
    
    if (!attachmentData.results || attachmentData.results.length === 0) {
      console.log(`No ${filename} found - first time setup`);
      return '';
    }
    
    const attachment = attachmentData.results[0];
...
async function uploadCSVToConfluence(filename, csvContent) {
  try {
    const formData = new FormData();
    const blob = new Blob([csvContent], { type: 'text/csv' });
    formData.append('file', blob, filename);
    formData.append('comment', 'Updated via Link Library');
...
function updateLinkCount() {
  ...
  const count = allLinks.filter(l => l.Link_Status === 'Active').length;
  countBadge.textContent = `${count} link${count !== 1 ? 's' : ''}`;
}
