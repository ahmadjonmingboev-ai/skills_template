...
const CSV_FILENAME_ANALYTICS = 'link_usage_analytics.csv';
...
function populateCategoryDropdowns() {
  ...
  const formDatalist = document.getElementById('categoryList');
  ...
  dynamicCategories.forEach(category => {
    ...
    formDatalist.appendChild(formOption);
  });
}
...
function displayCompactView() {
  const container = document.getElementById('linkCompact');
  const emptyState = document.getElementById('emptyState');
  ...
  container.innerHTML = linksToShow.map(link => createCompactCard(link)).join('');
}
...
function createCompactCard(link) {
  ...
  return `
    <div class="ll-compact-card" onclick="openDetailsModal('${link.Link_ID}')">
      <span class="ll-compact-badge ll-badge-category" style="background: ${categoryColor}; color: white;">
        ${escapeHtml(link.Link_Category)}
      </span>
      <div class="ll-compact-icon" style="background: ${categoryColor}20;">
        ${iconHtml}
      </div>
      <h4 class="ll-compact-title">${escapeHtml(link.Link_App_Name)}</h4>
      <div class="ll-compact-actions">
        ${hasProd 
          ? `<button class="ll-btn ll-btn-primary ll-btn-sm" onclick="openLinkUrl('${link.Link_ID}', 'prod'); event.stopPropagation();" title="PROD">
               <svg class="ll-icon"><use href="#icon-rocket"></use></svg>
             </button>`
          : `<button class="ll-btn ll-btn-na ll-btn-sm" disabled title="No PROD">
               P
             </button>`
        }
        ${hasUat 
          ? `<button class="ll-btn ll-btn-gray ll-btn-sm" onclick="openLinkUrl('${link.Link_ID}', 'uat'); event.stopPropagation();" title="UAT">
               <svg class="ll-icon"><use href="#icon-flask"></use></svg>
             </button>`
          : `<button class="ll-btn ll-btn-na ll-btn-sm" disabled title="No UAT">
               U
             </button>`
        }
      </div>
...
function openLinkModal(linkId = null) {
  ...
  if (linkId) {
    ...
    document.getElementById('linkAccessInstructions').value = link.Link_Access_Instructions || '';
    
    // Strip https:// prefix for display
    document.getElementById('linkAvatar').value = stripHttpsPrefix(link.Link_Avatar || '');
    document.getElementById('linkProd').value = stripHttpsPrefix(link.Link_Prod || '');
    document.getElementById('linkUat').value = stripHttpsPrefix(link.Link_UAT || '');
    
    updateCharCounter('linkDescription', 'descCharCount', 200);
    updateCharCounter('linkDetails', 'detailsCharCount', 1000);
    updateCharCounter('linkAccessInstructions', 'accessCharCount', 500);
  } else {
...
async function saveLink() {
  ...
  // Add https:// prefix if URL is provided
  ...
  const linkData = {
    ...
    Link_Category: document.getElementById('linkCategory').value.trim(),
    Link_Avatar: avatarUrl ? addHttpsPrefix(avatarUrl) : '',
    Link_Prod: prodUrl ? addHttpsPrefix(prodUrl) : '',
    Link_UAT: uatUrl ? addHttpsPrefix(uatUrl) : '',
    Link_Details: document.getElementById('linkDetails').value.trim(),
    Link_Access_Instructions: document.getElementById('linkAccessInstructions').value.trim(),
  ...
  try {
    ...
    const headers = ['Link_ID', 'Link_App_Name', 'Link_Description', 'Link_Category', 'Link_Avatar', 
                     'Link_Prod', 'Link_UAT', 'Link_Details', 'Link_Access_Instructions', 
                     'Link_Added_By', 'Link_Date_Added'];
...
function openDetailsModal(linkId) {
  ...
  // Track viewing details
  trackLinkAction(linkId, link.Link_App_Name, 'view_details', 'details');
  ...
  // Use requestAnimationFrame to defer heavy DOM updates
  requestAnimationFrame(() => {
    ...
    if (link.Link_Details && link.Link_Details.trim()) {
    ...
    // Access Instructions
    const accessSection = document.getElementById('detailsAccessSection');
    if (link.Link_Access_Instructions && link.Link_Access_Instructions.trim()) {
      document.getElementById('detailsAccessInstructions').textContent = link.Link_Access_Instructions;
    ...
    // Production URL
    const prodSection = document.getElementById('prodUrlSection');
    const openProdBtn = document.getElementById('openProdBtn');
    if (link.Link_Prod && link.Link_Prod.trim()) {
      document.getElementById('prodUrlText').textContent = stripHttpsPrefix(link.Link_Prod);
    ...
    // UAT URL
    const uatSection = document.getElementById('uatUrlSection');
    const openUatBtn = document.getElementById('openUatBtn');
    if (link.Link_UAT && link.Link_UAT.trim()) {
      document.getElementById('uatUrlText').textContent = stripHttpsPrefix(link.Link_UAT);
      uatSection.style.display = 'block';
      openUatBtn.style.display = 'inline-flex';
    } else {
      uatSection.style.display = 'none';
      openUatBtn.style.display = 'none';
    }
    
    // Metadata
    document.getElementById('detailsAddedBy').textContent = link.Link_Added_By || currentUserName;
...
function openLinkUrl(linkId, type) {
  ...
  if (url) {
    // Track the click
    trackLinkAction(linkId, link.Link_App_Name, `click_${type}`, type);
...
function shareLink(linkId) {
  ...
  // Track share action
  trackLinkAction(linkId, link.Link_App_Name, 'share', 'share');
...
async function exportLinks() {
  try {
    const headers = ['Link_ID', 'Link_App_Name', 'Link_Description', 'Link_Category', 'Link_Avatar', 
                     'Link_Prod', 'Link_UAT', 'Link_Details', 'Link_Added_By', 'Link_Date_Added'];
...
function setupEventListeners() {
  ...
  document.getElementById('linkAccessInstructions').addEventListener('input', () => {
    updateCharCounter('linkAccessInstructions', 'accessCharCount', 500);
  });
...
async function trackLinkAction(linkId, linkName, action, environment) {
  try {
    const logEntry = {
      Log_ID: `click-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
...
async function saveAnalytics() {
  try {
    ...
    const headers = ['Log_ID', 'Timestamp', 'Link_ID', 'Link_Name', 'User_Name', 'Action', 'Environment'];
...
function calculateAnalytics() {
  ...
  const last30Days = analyticsRecords.filter(r => {
  ...
  last30Days.forEach(record => {
    if (!clicksByLink[record.Link_ID]) {
      clicksByLink[record.Link_ID] = {
        id: record.Link_ID,
        name: record.Link_Name,
        count: 0
      };
    }
    clicksByLink[record.Link_ID].count++;
  });
  
  // Sort by count and get top 5
  const topLinks = Object.values(clicksByLink)
...
function openAnalyticsModal() {
  ...
  if (stats.topLinks.length === 0) {
    topLinksList.innerHTML = '<p class="ll-empty-analytics">No usage data in the last 30 days</p>';
  } else {
    const maxClicks = stats.topLinks[0].count;
    
    topLinksList.innerHTML = stats.topLinks.map((link, index) => {
      const percentage = (link.count / maxClicks) * 100;
      return `
        <div class="ll-top-link-item">
          <div class="ll-top-link-rank">${index + 1}</div>
          <div class="ll-top-link-name">${escapeHtml(link.name)}</div>
          <div class="ll-top-link-bar">
            <div class="ll-top-link-bar-fill" style="width: ${percentage}%"></div>
          </div>
          <div class="ll-top-link-count">${link.count} clicks</div>
        </div>
      `;
    }).join('');
  }
