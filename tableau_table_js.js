...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Load first tab - will wait for API to be ready via checkAndLoadFirstTab()
...
function loadTableauData(tabId) {
  ...
  try {
    ...
    tableContainer.innerHTML = '<div class="loading-spinner"><div class="spinner-icon">❌</div><p>Failed to load dashboard</p><p class="loading-hint">Error: ' + error.message + '</p><p class="loading-hint">Please check your permissions and sign in status</p></div>';
  }
}
...
function extractData(tabId, viz) {
  ...
  try {
    ...
    console.log('Extracting underlying data from worksheet:', worksheet.getName());
    
    // Get underlying data using reader (handles large datasets better)
    // Page size: 10000 rows per page
    extractDataWithReader(tabId, worksheet, containerNum);
    
  } catch (error) {
    console.error('Error in extractData for ' + tabId + ':', error);
    tableContainer.innerHTML = '<div class="loading-spinner"><div class="spinner-icon">❌</div><p>Error processing data</p><p class="loading-hint">' + error.message + '</p></div>';
  }
}

// Extract data using TableDataReader for better handling of large datasets
async function extractDataWithReader(tabId, worksheet, containerNum) {
  const tableContainer = document.getElementById('tableContainer' + containerNum);
  
  try {
    console.log('Initializing data reader for ' + tabId);
    
    // Get the reader with 10K rows per page
    const pageSize = 10000;
    const dataTableReader = await worksheet.getUnderlyingTableDataReaderAsync(pageSize);
    ...
    while (true) {
      ...
      tableContainer.innerHTML = '<div class="loading-spinner"><div class="spinner-icon">⏳</div><p>Loading data from Tableau...</p><p class="loading-hint">Page ' + pageNumber + ' (' + totalRowsLoaded.toLocaleString() + ' rows loaded so far)</p></div>';
      
      console.log('Reading page ' + pageNumber + ' for ' + tabId);
      
      // Get current page
      const currentPage = await dataTableReader.getPageAsync();
      const pageData = currentPage.getData();
      ...
      console.log('Loaded page ' + pageNumber + ': ' + pageData.length + ' rows (total: ' + totalRowsLoaded + ')');
      
      // Check if this is the last page
      if (pageData.length < pageSize) {
        console.log('Last page reached for ' + tabId);
        break;
      }
    }
    ...
    console.log('Data reader released for ' + tabId + '. Total rows: ' + totalRowsLoaded);
    ...
    console.log('Data extraction complete for ' + tabId + ': ' + rows.length + ' total rows');
    ...
    console.error('Error extracting data with reader for ' + tabId + ':', error);
    ...
    if (error.message && error.message.includes('permission')) {
      ...
      errorHint = 'Authentication required. Please sign in to Tableau Server first.';
    } else if (error.message && error.message.includes('getUnderlyingTableDataReaderAsync')) {
      errorHint = 'Your Tableau version may not support the data reader API. Please update Tableau or contact your admin.';
    }
    
    tableContainer.innerHTML = '<div class="loading-spinner"><div class="spinner-icon">❌</div><p>' + errorMessage + '</p><p class="loading-hint">' + errorHint + '</p><p class="loading-hint" style="font-size: 12px; margin-top: 12px;">Error details: ' + error.message + '</p></div>';
  }
}
