...
let tempFilters = {};
let columnWidths = {}; // Store custom column widths per tab
...
function extractData(tabId, viz) {
  ...
  try {
    ...
    worksheet.getUnderlyingDataAsync().then(function(dataTable) {
      console.log('Data extracted successfully for ' + tabId + ': ' + dataTable.getData().length + ' rows');
      ...
      // Convert to array of objects with formatted values
      const rows = data.map(row => {
        const obj = {};
        columns.forEach((col, idx) => {
          const colName = col.getFieldName();
          const rawValue = row[idx].value;
          obj[colName] = formatValue(rawValue, colName, tabId);
        });
        return obj;
      });
...
function renderTable(tabId) {
  ...
  // Headers with sorting, resizing, and drag-drop
  displayColumns.forEach((col, colIndex) => {
    ...
    const width = columnWidths[tabId] && columnWidths[tabId][col] ? `width="${columnWidths[tabId][col]}px"` : '';
    ...
    tableHTML += `<th class="sortable ${sortClass}" 
                    ...
                    <span class="resize-handle" 
                          onmousedown="startResize(event, '${tabId}', '${col}', ${colIndex})"
                          onclick="event.stopPropagation()"></span>
                  </th>`;
  });
  tableHTML += '</tr></thead><tbody>';
  
  // Rows (using custom column order)
...
function startResize(event, tabId, columnName, colIndex) {
...
function doResize(event) {
  ...
  const diff = event.pageX - resizeData.startX;
...
function stopResize(event) {
  ...
  const finalWidth = resizeData.th.offsetWidth;
  
  // Save the width for this column
  if (!columnWidths[resizeData.tabId]) {
    columnWidths[resizeData.tabId] = {};
  }
  columnWidths[resizeData.tabId][resizeData.columnName] = finalWidth;
