...
function showSignInWizard() {
  // Check if user is in sign-in session (sessionStorage flag with 2-hour expiry)
  const skipWizardTime = sessionStorage.getItem('skip_wizard_time');
  if (skipWizardTime) {
    const twoHoursInMs = 2 * 60 * 60 * 1000; // 2 hours in milliseconds
    const elapsed = Date.now() - parseInt(skipWizardTime);
    
    if (elapsed < twoHoursInMs) {
      console.log('Sign-in session active (within 2 hours), skipping wizard');
      return;
    } else {
      console.log('Sign-in session expired (>2 hours), clearing flag');
      sessionStorage.removeItem('skip_wizard_time');
    }
  }
...
function handleSignInWizardNo() {
  ...
  // Set sessionStorage flag with timestamp (valid for 2 hours)
  sessionStorage.setItem('skip_wizard_time', Date.now().toString());
  console.log('Set sessionStorage flag to skip wizard for 2 hours');
