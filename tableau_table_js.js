...
function loadTableauData(tabId) {
  ...
  try {
    ...
    // Keep the loading message, don't show error
  }
}
...
function openFilterModal(tabId) {
  ...
  data.columns.forEach(col => {
    ...
    const hasFilter = tempFilters[col] && tempFilters[col].trim();
    ...
    formHTML += `<select class="filter-select" id="filter_select_${col}" onchange="updateTempFilterFromSelect('${col}', this.value)">`;
    formHTML += `<option value="">All values</option>`;
    ...
    formHTML += `</select>`;
    formHTML += `</div>`;
    formHTML += `<input type="text" class="filter-input" placeholder="Or type to search..." value="${tempFilters[col] || ''}" oninput="updateTempFilterFromInput('${col}', this.value)">`;
    formHTML += `</div>`;
  });
...
// Update temp filter from select
function updateTempFilterFromSelect(column, value) {
...
function updateTempFilterFromInput(column, value) {
  ...
  // Update the select too if exact match
...
function closeSignInModal() {
  ...
  // Clear iframe
  document.getElementById('signInFrame').src = '';
}
