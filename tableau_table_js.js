...
function showSignInWizard() {
  // Check if URL has auth parameter (user just signed in and refreshed)
  const urlParams = new URLSearchParams(window.location.search);
  if (urlParams.has('auth')) {
    console.log('Auth parameter detected in URL, skipping wizard');
    // Clean up URL (remove parameter) without reloading page
...
function startSignInCountdown() {
  ...
  const countdown = setInterval(() => {
    ...
    if (seconds <= 0) {
      ...
      // Reload with auth parameter to skip wizard on next load
      const currentUrl = window.location.pathname;
      window.location.href = currentUrl + '?auth=done';
    }
  }, 1000);
