...
function loadTableauData(tabId) {
  ...
  try {
    ...
    // Check for authentication errors after a timeout
    setTimeout(() => {
      try {
        // Try to access the viz - if not authenticated, this will fail
        if (!loadedTabs.has(tabId)) {
          // Still loading or authentication required
          setTimeout(() => {
            if (!loadedTabs.has(tabId)) {
              // If still not loaded after 10 seconds, show auth prompt
    ...
    if (error.message && (error.message.includes('auth') || error.message.includes('login') || error.message.includes('credentials'))) {
      showAuthRequired(tabId);
    }
  }
}
...
function extractData(tabId, viz) {
  ...
  try {
    ...
    // Change from getUnderlyingDataAsync() to getSummaryDataAsync()
    worksheet.getSummaryDataAsync().then(function(dataTable) {
      ...
      if (error.message && (error.message.toLowerCase().includes('auth') || error.message.toLowerCase().includes('sign') || error.message.toLowerCase().includes('login'))) {
        showAuthRequired(tabId);
      }
    });
...
function showAuthRequired(tabId) {
  const config = dashboardConfig[tabId];
  const containerNum = tabId.replace('tab', '');
  const tableContainer = document.getElementById('tableContainer' + containerNum);
  ...
  const authHTML = `
    <div class="auth-required">
      ...
      <p>Please sign in to Tableau to view this dashboard data</p>
      <button class="signin-button" onclick="openTableauAuth('${tabId}')">
      ...
      <p style="margin-top: 16px; font-size: 12px;">After signing in, close the window and click "Retry Loading Data" below</p>
      <button class="signin-button" onclick="retryLoadData('${tabId}')" style="margin-top: 12px; background: #6b7280;">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
function openTableauAuth(tabId) {
  ...
  if (config && config.url) {
    // Open in new window for authentication
    window.open(config.url, 'TableauAuth', 'width=800,height=600');
  }
}
