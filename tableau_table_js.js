...
function showSignInWizard() {
  // Check if user is in sign-in session (sessionStorage flag)
  if (sessionStorage.getItem('skip_wizard') === 'true') {
    console.log('Sign-in session active (sessionStorage flag set), skipping wizard');
    return;
  }
...
function handleSignInWizardNo() {
  ...
  // Set sessionStorage flag to skip wizard after sign-in refresh
  sessionStorage.setItem('skip_wizard', 'true');
  console.log('Set sessionStorage flag to skip wizard on refresh');
...
function processAndStoreData(tabId, columns, data, containerNum) {
  ...
  // Clear sessionStorage flag since data loaded successfully
  // User is authenticated and working properly
  if (sessionStorage.getItem('skip_wizard') === 'true') {
    console.log('Data loaded successfully, clearing skip_wizard flag from sessionStorage');
    sessionStorage.removeItem('skip_wizard');
  }
...
function startSignInCountdown() {
  ...
  const countdown = setInterval(() => {
    ...
    if (seconds <= 0) {
      ...
      // Reload page (sessionStorage flag will skip wizard)
      location.reload();
    }
  }, 1000);
