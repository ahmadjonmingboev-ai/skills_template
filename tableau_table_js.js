...
function setupGlobal401Detection() {
  // Monitor console errors for 401 responses
  ...
  console.error = function(...args) {
    ...
    if (errorText.includes('401') || 
      ...
      console.log('401 error detected in console - opening sign-in modal');
      // Small delay to avoid opening modal multiple times
      setTimeout(() => {
  ...
  // Intercept fetch requests to detect 401
  const originalFetch = window.fetch;
  window.fetch = function(...args) {
    return originalFetch.apply(this, args)
      .then(response => {
        if (response.status === 401) {
          console.log('401 response detected from fetch - opening sign-in modal');
          setTimeout(() => {
            const modal = document.getElementById('signInModal');
            if (!modal.classList.contains('active')) {
              openSignInModal();
  ...
  // Intercept XMLHttpRequest to detect 401
  const originalXHROpen = XMLHttpRequest.prototype.open;
  XMLHttpRequest.prototype.open = function(...args) {
    this.addEventListener('load', function() {
      if (this.status === 401) {
        console.log('401 response detected from XHR - opening sign-in modal');
        setTimeout(() => {
          const modal = document.getElementById('signInModal');
          if (!modal.classList.contains('active')) {
            openSignInModal();
