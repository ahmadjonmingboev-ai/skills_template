...
function setupControlHandlers() {
  ...
  document.querySelectorAll('.rows-per-page').forEach(select => {
    select.addEventListener('change', function() {
      const tabId = this.getAttribute('data-tab');
      rowsPerPage[tabId] = parseInt(this.value);
      currentPage[tabId] = 1;
      renderTable(tabId);
    });
  });
...
function renderTable(tabId) {
  ...
  const totalPages = Math.ceil(totalRows / rowsPerPage[tabId]);
  const page = Math.min(currentPage[tabId], totalPages || 1);
  currentPage[tabId] = page;
  
  const startIdx = (page - 1) * rowsPerPage[tabId];
  const endIdx = Math.min(startIdx + rowsPerPage[tabId], totalRows);
