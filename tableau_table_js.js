...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Monitor for 401 errors globally (intercept fetch/XHR failures)
  setupGlobal401Detection();
...
// Setup global detection for 401/unauthorized errors
function setupGlobal401Detection() {
  let modalOpenPending = false;
  
  // Function to open modal with throttling
  function openSignInModalOnce() {
    if (modalOpenPending) return;
    modalOpenPending = true;
    
    const modal = document.getElementById('signInModal');
    if (!modal.classList.contains('active')) {
      console.log('Opening sign-in modal due to 401 error');
      openSignInModal();
    }
  ...
  // 1. Catch resource loading errors (for "Failed to load resource" errors)
  window.addEventListener('error', function(e) {
    // Resource loading errors appear here
    if (e.target && (e.target.tagName === 'IMG' || e.target.tagName === 'SCRIPT' || e.target.tagName === 'LINK')) {
      // This catches "Failed to load resource" - check if it's likely a 401
      console.log('Resource loading error detected, likely auth issue');
      setTimeout(openSignInModalOnce, 1000); // Wait 1 second to see if it's auth-related
    }
  }, true);
  
  // 2. Monitor console errors for 401 responses
  const originalError = console.error;
  console.error = function(...args) {
    // Check if any arguments contain 401/unauthorized
    const errorText = args.join(' ').toLowerCase();
    if (errorText.includes('401') || 
        errorText.includes('unauthorized') || 
        errorText.includes('not authorized')) {
      console.log('401 error detected in console');
      openSignInModalOnce();
    }
  ...
  // 3. Intercept fetch requests to detect 401
  if (window.fetch) {
      ...
      return originalFetch.apply(this, args)
        .then(response => {
          if (response.status === 401) {
            console.log('401 response detected from fetch');
            openSignInModalOnce();
          }
          return response;
        })
        .catch(error => {
          ...
          if (error && (String(error).includes('401') || String(error).includes('unauthorized'))) {
            console.log('401 error in fetch catch');
            openSignInModalOnce();
          }
          throw error;
        });
  ...
  // 4. Intercept XMLHttpRequest to detect 401
  const originalXHROpen = XMLHttpRequest.prototype.open;
  const originalXHRSend = XMLHttpRequest.prototype.send;
  
  XMLHttpRequest.prototype.open = function(...args) {
    this._url = args[1]; // Store URL for later
  ...
  XMLHttpRequest.prototype.send = function(...args) {
    this.addEventListener('load', function() {
      if (this.status === 401) {
        console.log('401 response detected from XHR:', this._url);
        openSignInModalOnce();
      }
    });
    this.addEventListener('error', function() {
      // Network errors might be 401
      console.log('XHR error, possibly auth-related:', this._url);
    });
    return originalXHRSend.apply(this, args);
  };
  
  // 5. Check for Tableau-specific auth failures
  // Tableau API might throw errors that we need to catch
  window.addEventListener('unhandledrejection', function(event) {
    const reason = String(event.reason).toLowerCase();
    if (reason.includes('401') || 
        reason.includes('unauthorized') || 
        reason.includes('authentication')) {
      console.log('401 detected in unhandled promise rejection');
      openSignInModalOnce();
    }
  });
}
...
function handleDataExtractionError(tabId, error, tableContainer) {
  ...
  // Check for 401/unauthorized errors immediately - these need sign-in
  const errorString = String(error).toLowerCase() + String(error.message || '').toLowerCase();
  ...
  if (is401Error) {
    console.log('401/Unauthorized error detected - opening sign-in modal');
    // Stop progress animation
    completeLoadingProgress(tabId);
    // Auto-open sign-in modal
    openSignInModal();
    return; // Don't show error message, just open sign-in
  }
