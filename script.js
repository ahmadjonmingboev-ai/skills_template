...
function generateTableHeaders() {
    ...
    tableColumns.forEach(column => {
        ...
        headerHTML += `<div class="grid-cell">${displayName}</div>`;
    });
    
    headerHTML += '<div class="grid-cell actions-col">Actions</div>';
...
function renderTable() {
    ...
    if (pageData.length === 0) {
        gridBody.innerHTML = '<div class="data-grid-empty">No data found. Try adjusting your filters.</div>';
    } else {
        ...
        pageData.forEach((rowData, index) => {
            ...
            let rowHTML = '';
            tableColumns.forEach(column => {
                rowHTML += `<div class="grid-cell" title="${rowData[column] || ''}">${rowData[column] || ''}</div>`;
            });
            
            // Add action buttons cell
            rowHTML += `
                <div class="grid-cell actions-col">
...
function openHistoryModal() {
    const historyGridBody = document.getElementById('historyGridBody');
    historyGridBody.innerHTML = '';
    
    if (historyData.length === 0) {
        historyGridBody.innerHTML = '<div class="data-grid-empty">No history records found.</div>';
    } else {
        // Use document fragment for better performance
        const fragment = document.createDocumentFragment();
        ...
        historyData.slice(0, 100).forEach(entry => {
            const row = document.createElement('div');
            row.className = 'history-grid-row';
            const timestamp = new Date(entry.Timestamp).toLocaleString();
            
            row.innerHTML = `
                <div class="history-col">${timestamp}</div>
                <div class="history-col">${entry.User}</div>
                ...
                <div class="history-col">${entry.RowID}</div>
            `;
            
            fragment.appendChild(row);
        });
        
        historyGridBody.appendChild(fragment);
    }
