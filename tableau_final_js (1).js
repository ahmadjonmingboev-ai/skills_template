...
// Load Tableau data - OPTION 1: VISIBLE VIZ FOR AUTHENTICATION
function loadTableauData(tabId) {
  ...
  tableContainer.innerHTML = `
    <div class="loading-spinner">
      ...
      <p class="loading-hint">If you see a sign-in prompt, please authenticate</p>
    </div>
  `;
  
  // Create a VISIBLE container for Tableau viz (for authentication)
  const vizDiv = document.createElement('div');
  vizDiv.style.width = '100%';
  vizDiv.style.minHeight = '600px';
  vizDiv.id = 'vizContainer_temp_' + containerNum;
  ...
  const options = {
    ...
    onFirstInteractive: function() {
      // Authentication successful and viz loaded!
      ...
      // Extract data after a brief delay
      setTimeout(() => {
        extractData(tabId, vizObjects[tabId], tableContainer);
      }, 500);
    }
  };
  
  // Append viz div to container
  tableContainer.appendChild(vizDiv);
  
  // Create viz (visible, so user can authenticate if needed)
  try {
    const viz = new tableau.Viz(vizDiv, config.url, options);
    vizObjects[tabId] = viz;
  } catch (error) {
    console.error('Error loading Tableau:', error);
    tableContainer.innerHTML = '<div class="loading-spinner"><p style="font-size: 18px;">❌</p><p>Error loading dashboard</p><p class="loading-hint">' + error.message + '</p></div>';
  }
}

// Extract data from Tableau
function extractData(tabId, viz, tableContainer) {
  const config = dashboardConfig[tabId];
  const containerNum = tabId.replace('tab', '');
  ...
  try {
    ...
    // Use options parameter to handle callback
    const options = {
      maxRows: 0, // 0 means get all rows
      ignoreAliases: false,
