...
function checkUserAuthorization() {
    // Authorization logic:
    // - If authorizedUsers list is empty = everyone is authorized (open access)
    // - If authorizedUsers list has usernames = only those users are authorized
    ...
    restrictedButtons.forEach(buttonId => {
        ...
        if (button) {
            if (isAuthorized) {
                // Show and enable button for authorized users
...
function renderTable() {
    ...
    // Add UPDATED_BY and UPDATED_TIME as visible columns if custom columns exist
    const systemCustomColumns = [];
    if (customColumns.length > 0) {
        systemCustomColumns.push('UPDATED_BY', 'UPDATED_TIME');
    }
    
    const customColumnNames = customColumns.map(c => c.name);
    const allCustomColumns = [...systemCustomColumns, ...customColumnNames];
    const allColumns = [...allCustomColumns, ...orderedDataColumns];
...
function renderRow(row, globalIndex, tbody, allColumns, customColumnsData, tab, customColumnNames, allCustomColumns) {
    ...
    // Render system custom columns first (UPDATED_BY, UPDATED_TIME)
    const systemColumns = ['UPDATED_BY', 'UPDATED_TIME'];
    const customRow = customColumnsData.find(c => c[tab.primaryKey] == primaryKeyValue);
    
    // Only show system columns if custom columns exist
    if (tab.customColumns && tab.customColumns.length > 0) {
    ...
    customColumnNames.forEach(colName => {
        ...
        // Don't show red line for empty custom columns (removed the required field indicator)
...
function updateButtonVisibility() {
    ...
    // Only show buttons if user is authorized AND tab has writeBack enabled
    if (isAuthorized && tab.writeBack) {
    ...
    if (isAuthorized && tab.customColumns && tab.customColumns.length > 0) {
