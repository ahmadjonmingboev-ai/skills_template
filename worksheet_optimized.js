...
function applyAutoRefresh(value, label) {
...
function initAutoRefresh() {
    ...
    const savedLabel = sessionStorage.getItem('autoRefreshLabel') || '';
    if (saved) {
        applyAutoRefresh(saved, savedLabel);
    }
}
...
function setupEventListeners() {
    ...
    document.getElementById('autoRefreshButton')?.addEventListener('click', function(e) {
        e.stopPropagation();
        document.getElementById('autoRefreshDropdown').classList.toggle('show');
    });

    // Auto-refresh option selection
    document.querySelectorAll('.auto-refresh-option').forEach(opt => {
        opt.addEventListener('click', function() {
            applyAutoRefresh(this.dataset.value, this.dataset.label);
        });
    });
    ...
    window.addEventListener('click', function(e) {
        ...
        const wrapper = document.getElementById('autoRefreshWrapper');
        if (wrapper && !wrapper.contains(e.target)) {
            document.getElementById('autoRefreshDropdown')?.classList.remove('show');
        }
    });
}
