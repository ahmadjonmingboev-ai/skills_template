    ...
    try {
        ...
        reader.onload = async function(e) {
            try {
                ...
                if (!tab || !tab.filename) {
                    showToast('Please specify a filename for the tab', 'error');
                    return;
                }
                
                // Upload to Confluence attachments
                await uploadAttachment(tab.filename, content);
                
                showToast(`File uploaded successfully for ${tab.displayName}`, 'success');
                
                // Reload tab data if current
            ...
            item.querySelectorAll('.custom-column-config').forEach(colConfig => {
                ...
                if (colName) {
                    customColumns.push({
                        ...
                        statusColors: colColor // For dropdown status colors
                    });
                }
            });
                ...
                <div class="custom-columns-container" id="customColumns_${index}">
                    ${tab.customColumns ? tab.customColumns.map((col, colIndex) => `
                        <div class="custom-column-config">
                            ...
                            <select class="custom-col-color status-color-select" ${col.type !== 'dropdown' ? 'style="display:none"' : ''}>
                                <option value="">Default</option>
                                <option value="status-colors" ${col.statusColors === 'status-colors' ? 'selected' : ''}>Status Colors</option>
                            </select>
                            <label>
...
function addCustomColumn(tabIndex) {
    ...
    colConfig.innerHTML = `
        ...
        <select class="custom-col-color status-color-select" style="display:none">
            <option value="">Default</option>
    ...
    colConfig.querySelector('.custom-col-type').addEventListener('change', function() {
        const colorSelect = colConfig.querySelector('.custom-col-color');
        colorSelect.style.display = this.value === 'dropdown' ? '' : 'none';
    });
