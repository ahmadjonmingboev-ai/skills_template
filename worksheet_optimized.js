...
async function updatePresence() {
    try {
        ...
        // Remove old entries (older than 2 minutes = 120 seconds)
        // Logic: If someone hasn't updated in 2 minutes, consider them gone
        const twoMinutesAgo = new Date(Date.now() - 120000).toISOString();
        presenceData = presenceData.filter(entry => entry.TIMESTAMP > twoMinutesAgo);
...
function updatePresenceUI() {
    ...
    if (STATE.activeUsers.length > 0) {
        ...
        `;
        presenceIndicator.title = 'Currently viewing:\n' + STATE.activeUsers.join('\n');
