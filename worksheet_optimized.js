...
async function init() {
    try {
        ...
        try {
            ...
            console.warn('Config not found, will create on first save');
...
function autoDetectConfluenceContext() {
    // Try to auto-detect from window location
    if (!CONFIG.confluenceBaseUrl) {
        CONFIG.confluenceBaseUrl = window.location.origin + '/wiki';
    }
    ...
    console.log('Auto-detected Confluence context:', {
...
async function updatePresence() {
    try {
        ...
        presenceData = presenceData.filter(entry => entry.TIMESTAMP > thirtySecondsAgo);
...
function updatePresenceUI() {
    ...
    if (STATE.activeUsers.length > 0) {
        ...
        presenceIndicator.innerHTML = `
            ...
            <span>${STATE.activeUsers.length} viewing</span>
        `;
        presenceIndicator.title = STATE.activeUsers.join(', ');
...
function startPresenceTracking() {
    ...
    STATE.presenceInterval = setInterval(updatePresence, 15000);
}
...
function renderTable() {
    ...
    // Add system columns (UPDATED_BY, UPDATED_TIME) if custom columns exist
    const systemColumns = [];
    if (customColumns.length > 0) {
    ...
    const allCustomColumns = [...systemColumns, ...customColumnNames];
...
function renderRow(row, globalIndex, tbody, allColumns, customColumnsData, tab, customColumnNames, allCustomColumns) {
    ...
    // Render system columns and custom columns
    const customRow = customColumnsData.find(c => c[tab.primaryKey] == primaryKeyValue);
    
    // Render UPDATED_BY and UPDATED_TIME if custom columns exist
    if (tab.customColumns && tab.customColumns.length > 0) {
...
function openConfigModal() {
    ...
    if (!CONFIG.confluenceBaseUrl || !CONFIG.pageId) {
        autoDetectConfluenceContext();
    }
