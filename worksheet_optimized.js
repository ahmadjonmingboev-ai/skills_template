...
async function init() {
    try {
        ...
        try {
            ...
            // Restore auto-refresh from previous session
            initAutoRefresh();
...
function startPresenceTracking() {
    // Write presence only once on initial page load.
    // No interval — repeated uploads create new Confluence attachment versions
    // and trigger notifications to watchers on every poll.
    updatePresence();
}
...
function renderTable() {
    ...
    if (STATE.groupByColumn && Object.keys(groups).length > 0) {
        // Group only the rows on the current page so pagination works correctly.
        // `groups` (built from all data) is still used for the total-count label.
        const pageGroups = {};
        pageData.forEach((row, localIndex) => {
            ...
            if (!pageGroups[groupValue]) pageGroups[groupValue] = [];
            pageGroups[groupValue].push({ row, globalIndex: startIndex + localIndex });
        });

        Object.entries(pageGroups).forEach(([groupName, items]) => {
            const isCollapsed = STATE.collapsedGroups.has(groupName);
            const totalInGroup = groups[groupName] ? groups[groupName].length : items.length;
            ...
            groupHeader.innerHTML = `
                <td colspan="${allColumns.length + 1}">
                    <button class="group-toggle-btn" onclick="toggleGroup('${escapeHtml(groupName)}')">
                        ${isCollapsed ? '▶' : '▼'} ${escapeHtml(groupName)} (${totalInGroup} items)
                    </button>
                </td>
            `;
            ...
            if (!isCollapsed) {
                items.forEach(({ row, globalIndex }) => {
                    renderRow(row, globalIndex, tbody, allColumns, customColumnsData, tab, customColumnNames, allCustomColumns);
                });
            }
        });
    } else {
...
// Shared resize state — lives outside any function so both the mousedown
// handler (set per-render) and the single document-level mousemove/mouseup
// handler (set once) can read and write the same object.
const _resizeState = {
    active: false,
    startX: 0,
    startWidth: 0,
...
let _resizeListenersReady = false;

// Called once to attach the document-level move/up listeners.
// Safe to call multiple times — the flag prevents double-registration.
function initColumnResizeListeners() {
    if (_resizeListenersReady) return;
    _resizeListenersReady = true;

    document.addEventListener('mousemove', function(e) {
        if (!_resizeState.active || !_resizeState.th) return;
        const newWidth = Math.max(100, _resizeState.startWidth + (e.pageX - _resizeState.startX));
        _resizeState.th.style.width    = newWidth + 'px';
        _resizeState.th.style.minWidth = newWidth + 'px';
        _resizeState.th.style.maxWidth = newWidth + 'px';
    });

    document.addEventListener('mouseup', function() {
        if (!_resizeState.active) return;
        _resizeState.active = false;
        if (_resizeState.handle) _resizeState.handle.classList.remove('resizing');
        const table = document.getElementById('wsDataTable');
        if (table) table.classList.remove('resizing');
        document.body.style.cursor    = '';
        document.body.style.userSelect = '';
        _resizeState.th     = null;
        _resizeState.handle = null;
    });
}

// Called on every renderTable() — only wires up the per-handle mousedown.
// No document listeners are added here, so there is no leak.
function setupColumnResize() {
    ...
    resizeHandles.forEach(handle => {
        handle.onmousedown = function(e) {
            ...
            _resizeState.th          = this.parentElement;
            _resizeState.startX      = e.pageX;
            _resizeState.startWidth  = this.parentElement.offsetWidth;
            _resizeState.handle      = this;
...
function initAutoRefresh() {
    const saved = sessionStorage.getItem('autoRefreshInterval');
    const select = document.getElementById('autoRefreshSelect');
    if (saved && select) {
        select.value = saved;
        if (saved) startAutoRefresh(parseInt(saved, 10));
    }
}

// ===== EVENT LISTENERS =====
function setupEventListeners() {
    initColumnResizeListeners();
    ...
    document.getElementById('autoRefreshSelect')?.addEventListener('change', function() {
        ...
        if (value) {
            sessionStorage.setItem('autoRefreshInterval', value);
            startAutoRefresh(parseInt(value, 10));
        } else {
