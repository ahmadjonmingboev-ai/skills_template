...
async function loadData() {
    try {
        ...
        // Clear saved modal form data on refresh
        clearModalFormData();
...
function openEditModal(index) {
    ...
    // Restore saved form data if exists (in case user closed and reopened)
    restoreModalFormData();
...
function saveModalFormData() {
    ...
    sessionStorage.setItem('modalFormData', JSON.stringify(formData));
}

// Restore form data from sessionStorage
function restoreModalFormData() {
    const savedData = sessionStorage.getItem('modalFormData');
    if (savedData) {
        try {
            ...
            tableColumns.forEach(column => {
                const fieldId = `edit${column}`;
                const field = document.getElementById(fieldId);
                if (field && formData[column] !== undefined) {
                    field.value = formData[column];
                }
            });
        } catch (error) {
...
function setupAutoSaveFormData() {
    tableColumns.forEach(column => {
        ...
        const field = document.getElementById(fieldId);
        if (field) {
            field.removeEventListener('input', saveModalFormData); // Remove old listener
            field.addEventListener('input', saveModalFormData); // Add new listener
        }
    });
}

// Clear saved form data (called on successful save or refresh)
function clearModalFormData() {
    sessionStorage.removeItem('modalFormData');
}
...
async function saveRecord() {
    ...
    try {
        ...
        // CHANGE 1: Handle comma-separated category codes - declare outside if/else
        const categoryCol = tableColumns.find(col => col.includes('CATEGORY')) || 'CATEGORY_CODE';
        let categoryCodes = [newRecord[categoryCol]]; // Default to single code
        ...
        if (isEdit) {
            ...
            // Handle comma-separated category codes
            categoryCodes = newRecord[categoryCol] ? newRecord[categoryCol].split(',').map(c => c.trim()).filter(c => c) : [newRecord[categoryCol]];
        ...
        // Clear the modal form data after successful save
        sessionStorage.removeItem('modalFormData');
...
function generateString() {
    ...
    // CHANGE 2: Team Group generation using Team_Group column
    if (generateFor === 'team') {
        const teamGroupCol = tableColumns.find(col => col === 'Team_Group' || col === 'TEAM_GROUP') || 'Team_Group';
        const categoryCol = tableColumns.find(col => col.includes('CATEGORY')) || 'CATEGORY_CODE';
        const branchCol = tableColumns.find(col => col.includes('BRANCH')) || 'BRANCH_CODE';
        
        // Group by Team_Group value
        const teamGroups = {};
        dataToProcess.forEach(row => {
            const teamGroup = row[teamGroupCol];
            if (!teamGroup) return;

            if (!teamGroups[teamGroup]) {
            ...
            if (branchKey) {
                teamGroups[teamGroup].push(branchKey);
            }
        });
        ...
        teams.forEach((team, index) => {
            const branches = teamGroups[team];
