...
async function initializeApp() {
    ...
    try {
        ...
        // Load main workbook for template data BEFORE checking admin access
        // This ensures mainWorkbook is loaded for all operations
        await loadMainWorkbook();
        
        // Check if user is admin (this also loads user data)
        ...
        // Load user data again to ensure it's fresh
        await loadUserData();
...
async function loadMainWorkbook() {
    try {
        ...
        if (templateSheet) {
            // Load template data with all columns, including empty cells
            templateData = XLSX.utils.sheet_to_json(templateSheet, { defval: '' });
            
            // Identify all columns in template
            if (templateData.length > 0) {
                const allColumns = Object.keys(templateData[0]);
                const standardColumns = ['Item_ID', 'Category', 'Task_Group', 'Task_Name'];
                const marketColumns = allColumns.filter(col => !standardColumns.includes(col));
                console.log('Template loaded with', templateData.length, 'tasks');
                console.log('Standard columns:', standardColumns);
                console.log('Market columns:', marketColumns);
            }
        } else {
            console.warn('Template sheet not found in main workbook');
            templateData = [];
        }
    } catch (error) {
...
async function saveUser() {
    ...
    // For new users, verify template exists and has tasks
    if (!isEditMode) {
        if (!templateData || templateData.length === 0) {
            // Try to reload the main workbook
            await loadMainWorkbook();
            if (!templateData || templateData.length === 0) {
                showToast('Template sheet is missing or empty. Please ensure Template sheet exists in maintaskdatabase.xlsx', 'error');
                return;
            }
        }
    }
    ...
    try {
        ...
        showToast(`Failed to ${isEditMode ? 'update' : 'add'} user: ${error.message}`, 'error');
    } finally {
        showLoading(false);
    }
}
...
async function addNewUser(userData) {
    ...
    const userOut = XLSX.write(userWorkbook, { 
        bookType: 'xlsx', 
        type: 'array',
        compression: true,
        bookSST: true  // Shared string table to reduce size
    });
    ...
    // Get fresh template data and identify all market columns
    const templateSheet = mainWorkbook.Sheets['Template'];
    if (!templateSheet) {
        throw new Error('Template sheet not found in main workbook');
    }
    
    // Get all data from template including all columns
    const templateDataFull = XLSX.utils.sheet_to_json(templateSheet, { defval: '' });
    
    // Identify market columns (all columns except standard ones)
    const standardColumns = ['Item_ID', 'Category', 'Task_Group', 'Task_Name'];
    let marketColumns = [];
    if (templateDataFull.length > 0) {
        marketColumns = Object.keys(templateDataFull[0]).filter(col => !standardColumns.includes(col));
    }
    
    console.log('Creating user sheet with', templateDataFull.length, 'tasks and markets:', marketColumns);
    
    // Create new sheet data with all tasks and markets set to 'no'
    const newSheetData = templateDataFull.map(task => {
        const userTask = {};
        
        // Copy standard task columns
        standardColumns.forEach(col => {
            userTask[col] = task[col] || '';
        });
        
        // Add all market columns with 'no' value
        marketColumns.forEach(market => {
            userTask[market] = 'no';
        });
    ...
    // Verify data before creating sheet
    if (newSheetData.length === 0) {
        throw new Error('No tasks found in template sheet');
    }
    
    console.log('First task for new user:', newSheetData[0]);
    
    // Create new sheet with proper column order and minimal formatting
    const columnOrder = [...standardColumns, ...marketColumns];
    const newSheet = XLSX.utils.json_to_sheet(newSheetData, { 
        header: columnOrder,
        skipHeader: false
    });
    
    // Remove any default styles/formatting to reduce size
    delete newSheet['!cols'];
    delete newSheet['!rows'];
    delete newSheet['!merges'];
    delete newSheet['!protect'];
    delete newSheet['!autofilter'];
    ...
    // Clean up workbook metadata to reduce size
    delete mainWorkbook.Workbook?.Names;
    delete mainWorkbook.Workbook?.Views;
    if (mainWorkbook.Props) {
        mainWorkbook.Props = {
            Title: "Task Database",
            LastAuthor: currentUser.username
        };
    }
    
    // Save main workbook with maximum compression
    const mainOut = XLSX.write(mainWorkbook, { 
        bookType: 'xlsx', 
        type: 'array',
    ...
    console.log(`User sheet created for ${userData.username} with ${newSheetData.length} tasks`);
}
...
async function updateExistingUser(originalUsername, userData) {
    ...
    if (originalUsername !== userData.username) {
        ...
        if (oldSheet) {
            ...
            // Clean up workbook metadata
            delete mainWorkbook.Workbook?.Names;
            delete mainWorkbook.Workbook?.Views;
            
            // Save main workbook with compression
            const mainOut = XLSX.write(mainWorkbook, { 
                bookType: 'xlsx', 
                type: 'array',
                compression: true,
    ...
    try {
        ...
        try {
            ...
            if (approvals.length < originalCount) {
                ...
                const approvalsOut = XLSX.write(approvalsWorkbook, { 
                    bookType: 'xlsx', 
                    type: 'array',
                    compression: true,
                    bookSST: true
...
async function logAudit(action, itemId, details, previousValue = '', newValue = '') {
    try {
        ...
        // Save audit log with compression
