...
let inspirationView = false; // not persisted; resets on refresh
...
function setupEventListeners() {
  ...
  document.getElementById('cardStyleToggle').addEventListener('click', toggleInspirationView);
...
function displayGridView(videos) {
  ...
  gridEl.innerHTML = videos.map((video, index) => inspirationView ? renderAltCard(video, index) : renderDefaultCard(video, index)).join('');
}

function renderDefaultCard(video, index) {
  ...
  return `
    <div class="vl-video-card" style="animation-delay: ${index * CARD_STAGGER_DELAY}ms">
      <div class="vl-video-thumbnail">
        ...
        <div class="vl-card-overlay" onclick="handleWatch('${video.Video_ID}')">
          <div class="vl-card-overlay-btn">
            <svg class="vl-icon vl-icon-sm"><use href="#icon-play"></use></svg>
            Watch
          </div>
        </div>
...
function renderFooterActions(video) {
  return `
...
function getThumbnailUrl(video) {
  if (video.Video_Thumbnail) return video.Video_Thumbnail;
  return '';
}
...
function setFocusVideo(videoId) {
  ...
  meta.innerHTML = `
    ...
    ${video.Video_Duration ? `<div class="vl-video-meta-item"><svg class="vl-icon vl-icon-sm"><use href="#icon-clock"></use></svg>${video.Video_Duration}</div>` : ''}
  `;
...
function buildFocusRail(activeId) {
  ...
  if (!rail) return;
  rail.innerHTML = filteredVideos.map(v => {
...
async function fetchUserAvatar(username) {
  if (!username || userAvatarCache[username]) {
    return userAvatarCache[username] || null;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (userData.profilePicture && userData.profilePicture.path) {
        ...
        // Re-render if avatar loaded after initial display
        setTimeout(() => {
          updateAvatarInDOM(username, avatarUrl);
        }, 100);
...
function updateAvatarInDOM(username, avatarUrl) {
  // Find all avatar placeholders for this user and update them
  const cards = document.querySelectorAll('.vl-video-card, .vl-list-item');
  cards.forEach(card => {
    const metaItem = card.querySelector('.vl-video-meta-item');
    if (metaItem && metaItem.textContent.includes(username)) {
      const icon = metaItem.querySelector('.vl-icon');
      if (icon && avatarUrl) {
        const img = document.createElement('img');
        img.src = avatarUrl;
        img.alt = username;
        img.className = 'vl-user-avatar';
