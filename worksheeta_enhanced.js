// WORKSHEET TOOL - Enhanced Version v5 with All Fixes and Features
// ==================================================================

// ===== SECURITY: ENHANCED HTML ESCAPING (XSS Protection) =====
function escapeHtml(text) {
    ...
    // Additional escaping for complete XSS protection
...
const CONFIG = {
    ...
    theme: 'auto',
    authorizedUsers: [] // New: list of authorized users
};
...
const STATE = {
    ...
    sortColumn: null,
    sortDirection: null,
    modalData: {}, // New: Store modal data for persistence
    rowLocks: {}, // New: Track row locks
    changeHistory: [], // New: Track changes
    groupByColumn: null, // New: For grouping feature
    collapsedGroups: new Set() // New: Track collapsed groups
};

// ===== INITIALIZATION =====
document.addEventListener('DOMContentLoaded', async function() {
    console.log('Worksheet Tool Enhanced v5 initializing...');
    await init();
});
...
async function init() {
    try {
        ...
        try {
            ...
            console.warn('Config not found, opening configuration modal');
            showLoading(false);
        ...
        setupEventListeners();
        checkUserAuthorization(); // New: Check user permissions
...
function checkUserAuthorization() {
    const isAuthorized = CONFIG.authorizedUsers.length === 0 || 
                         CONFIG.authorizedUsers.includes(STATE.currentUser);
    
    // Disable/enable buttons based on authorization
    const restrictedButtons = [
        'addRowButton', 'editRowButton', 'deleteRowsButton', 
        'addCommentsButton', 'duplicateRowButton'
    ];
    
    restrictedButtons.forEach(buttonId => {
        const button = document.getElementById(buttonId);
        if (button) {
            button.style.display = isAuthorized ? 'inline-flex' : 'none';
        }
    });
}
...
async function handleFileUpload(tabIndex, file) {
    try {
        ...
        reader.onload = async function(e) {
            ...
            await uploadAttachment(tab.filename, content);
            
            showToast(`File uploaded successfully for ${tab.displayName}`, 'success');
            
            // Reload tab data
            delete STATE.allData[tab.name];
            if (STATE.currentTab === tab.name) {
                await switchTab(tab.name);
            }
        };
...
async function loadConfiguration() {
    ...
    configData.forEach(row => {
        ...
        if (field === 'WORKSHEET_NAME') {
            ...
            CONFIG.authorizedUsers = value.split(',').map(u => u.trim());
            ...
            if (!STATE.tabs.find(t => t.key === tabKey)) {
                STATE.tabs.push({
                    ...
                    displayName: value,
                    filename: '',
                    primaryKey: 'ID', // Tab-level primary key
                    sumColumn: '',
                    writeBack: false,
                    customColumns: [], // Tab-level custom columns
                    columnOrder: [],
                    groupByOptions: [], // Columns available for grouping
                    assignmentRules: [] // Assignment rules for this tab
                });
            }
        ...
        } else if (field.match(/^TAB_FILE_\d+_ASSIGNMENT_RULES$/)) {
            const tabNum = field.split('_')[2];
            const tab = STATE.tabs.find(t => t.key === `TAB_${tabNum}`);
            if (tab && value) {
                try {
                    tab.assignmentRules = JSON.parse(value);
                } catch (e) {
                    console.warn('Failed to parse assignment rules:', e);
                }
            }
        }
    });
...
async function saveConfiguration() {
    try {
        ...
        for (let index = 0; index < tabConfigs.length; index++) {
            ...
            const primaryKey = item.querySelector('.tab-primary-key-input').value.trim();
        ...
        STATE.tabs.forEach((tab, index) => {
            ...
            if (tab.assignmentRules.length > 0) {
                configRows.push({ Fields: `TAB_FILE_${index + 1}_ASSIGNMENT_RULES`, Values: JSON.stringify(tab.assignmentRules) });
            }
        });
...
// ===== CUSTOM COLUMNS FILE MANAGEMENT =====
...
// FIXED: Load tab data with proper custom columns handling
...
// FIXED: Sort function that works with custom columns
function sortColumn(column) {
    ...
    if (STATE.sortColumn && STATE.sortDirection) {
        tabData.filteredData.sort((a, b) => {
            ...
            if (isCustomColumn) {
                // Get values from custom columns data
...
// FIXED: Enhanced table rendering with proper custom columns
function renderTable() {
    ...
    // Combine all columns
    const allColumns = [...customColumnNames, ...orderedDataColumns];
    ...
    if (STATE.groupByColumn) {
        groups = {};
        data.forEach(row => {
            const groupValue = row[STATE.groupByColumn] || 'Ungrouped';
    ...
    thead.innerHTML = `
        <tr>
            ...
            ${allColumns.map((col) => {
                const isCustomColumn = customColumnNames.includes(col);
                const sortClass = STATE.sortColumn === col 
                    ? (STATE.sortDirection === 'asc' ? 'sort-asc' : 'sort-desc')
                    : 'sortable';
                return `<th class="${sortClass}" data-column="${escapeHtml(col)}">${escapeHtml(col)}</th>`;
            }).join('')}
        </tr>
    `;
    ...
    if (pageData.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="${allColumns.length + 2}" style="text-align: center; padding: 40px;">
                    No data available
                </td>
            </tr>
        `;
    ...
    // Render grouped or regular data
        ...
        Object.entries(groups).forEach(([groupName, groupRows]) => {
            ...
            if (!isCollapsed) {
                groupRows.slice(startIndex, Math.min(startIndex + pageSize, groupRows.length)).forEach((row, index) => {
                    renderRow(row, startIndex + index, tbody, allColumns, customColumnsData, tab, customColumnNames);
                });
            }
        });
    } else {
...
function renderRow(row, globalIndex, tbody, allColumns, customColumnsData, tab, customColumnNames) {
    ...
    actionsTd.innerHTML = `
        <div class="row-actions">
            ${isLocked ? '<span class="lock-indicator" title="Row is locked">🔒</span>' : ''}
            <button class="btn-icon" onclick="duplicateRow(${globalIndex})" title="Duplicate">
    ...
    customColumnNames.forEach(colName => {
        ...
        if (customCol && customCol.type === 'dropdown' && customCol.options.length > 0) {
            td.innerHTML = `<span class="dropdown-value">${escapeHtml(value)}</span>`;
        } else if (customCol && customCol.type === 'date') {
            td.innerHTML = `<span class="date-value">${escapeHtml(value)}</span>`;
        } else if (customCol && customCol.type === 'user') {
            td.innerHTML = `<span class="user-value">${escapeHtml(value)}</span>`;
        } else {
            td.textContent = value;
        }
        
        if (customCol && customCol.required && !value) {
            td.classList.add('missing-required');
        }
    ...
    allColumns.forEach(col => {
        if (!customColumnNames.includes(col)) {
            const td = document.createElement('td');
            const value = row[col];
...
function duplicateRow(index) {
    const tabData = STATE.allData[STATE.currentTab];
    const tab = STATE.tabs.find(t => t.name === STATE.currentTab);
    const row = tabData.filteredData[index];
    
    if (!row) return;
    
    // Create a copy with new primary key
    const newRow = { ...row };
...
async function lockRow(primaryKey) {
    try {
        ...
        // Broadcast lock to other users (simplified - would use WebSocket in production)
        await broadcastLock(primaryKey, true);
...
async function unlockRow(primaryKey) {
    try {
        ...
        await broadcastLock(primaryKey, false);
        return true;
    } catch (error) {
...
async function broadcastLock(primaryKey, isLocked) {
    // In production, this would use WebSocket or similar
    console.log(`Row ${primaryKey} ${isLocked ? 'locked' : 'unlocked'} by ${STATE.currentUser}`);
}
...
async function saveChangeHistory() {
    try {
        const historyFile = 'change_history.csv';
...
async function loadChangeHistory() {
    try {
        ...
        STATE.changeHistory = CSV.parse(historyText);
    } catch (error) {
...
// FIXED: Comments modal with persistence and previous values
function openCommentsModal() {
    ...
    // Load previous modal state
    const modalState = loadModalState('commentModal');
    
    // Get existing values for selected rows
    ...
    // Show custom columns with proper types
    tab.customColumns.forEach(col => {
        ...
        if (col.type === 'dropdown' && col.options.length > 0) {
            ...
            inputHtml = `
                <input type="text" class="form-control comment-field user-autocomplete" 
                       data-column="${escapeHtml(col.name)}" 
                       value="${escapeHtml(savedValue)}"
...
function closeCommentModal() {
    ...
    // Don't clear modal state here - preserve it
}

// FIXED: Save comments with all columns including the last one
async function saveComments() {
    try {
        ...
        document.querySelectorAll('#commentFormContainer .comment-field').forEach(field => {
            ...
            // IMPORTANT FIX: Ensure all columns are captured including the last one
        ...
        commentData['UPDATED_TIME'] = new Date().toLocaleString('en-GB', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
            hour: '2-digit',
        ...
        // Ensure customColumnsData exists
        ...
        // IMPORTANT: Ensure all columns are included when generating CSV
        const csvContent = CSV.generate(tabData.customColumnsData);
        await uploadAttachment(customFileName, csvContent);
        
        showToast('Custom columns saved successfully!', 'success');
...
// Enhanced Edit Row Modal with persistence
...
function closeEditRowModal() {
    ...
    // Don't clear modal state - preserve it
}
...
async function saveEditedRow() {
    try {
        ...
        if (hasCustomData && tab.customColumns.length > 0) {
            ...
            customData['UPDATED_TIME'] = new Date().toLocaleString();
...
// [The code continues with all other functions like deleteSelectedRows, logAudit, etc.]
// Due to length limits, I'll include the most critical remaining functions:
...
async function logAudit(action, tabName, primaryKey) {
    try {
        ...
        try {
            const auditText = await fetchAttachment('audit_log.csv');
        ...
        await uploadAttachment('audit_log.csv', csvContent);
    } catch (error) {
...
function setupEventListeners() {
    ...
    document.getElementById('duplicateRowButton').addEventListener('click', () => {
        if (STATE.selectedRows.size === 1) {
            duplicateRow(Array.from(STATE.selectedRows)[0]);
        } else {
...
function updateSummary() {
    ...
    // Show negative sum in red
    if (sum < 0) {
        sumElement.style.color = 'var(--danger)';
    } else {
...
async function deleteSelectedRows() {
    ...
    showConfirm(`Delete ${STATE.selectedRows.size} row(s)?`, async function() {
        try {
            ...
            tabData.rawData = tabData.rawData.filter(r => !primaryKeysToDelete.includes(r[tab.primaryKey]));
            ...
            <div class="custom-columns-section">
                ...
                <div class="custom-columns-container" id="customColumns_${index}">
                    ${tab.customColumns ? tab.customColumns.map((col, colIndex) => `
                        <div class="custom-column-config">
                            ...
                            <select class="custom-col-type">
                                ...
                                <option value="user" ${col.type === 'user' ? 'selected' : ''}>User</option>
                            </select>
...
function openFilterModal() {
    ...
    // Get all columns including custom
...
function openHistoryModal() {
    ...
    const recentHistory = STATE.changeHistory.slice(-50).reverse();
    
    if (recentHistory.length === 0) {
        container.innerHTML = '<p>No change history available</p>';
    } else {
        ...
        table.innerHTML = `
            ...
            <tbody>
                ${recentHistory.map(change => `
                    <tr>
                        <td>${escapeHtml(new Date(change.timestamp).toLocaleString())}</td>
                        <td>${escapeHtml(change.user)}</td>
                        <td>${escapeHtml(change.action)}</td>
                        <td>${escapeHtml(change.tab)}</td>
                        <td>${escapeHtml(change.primaryKey)}</td>
                    </tr>
                `).join('')}
            </tbody>
        `;
...
console.log('Worksheet Tool Enhanced v5 loaded!');
