// WORKSHEET TOOL - Final Version v4 with All Critical Fixes
// ==========================================================
...
document.addEventListener('DOMContentLoaded', async function() {
    console.log('Worksheet Tool v4 initializing...');
    await init();
});
...
// FIX #4: COMMENTS - Dynamic extra columns
async function loadTabData(tabName) {
    ...
    let comments = [];
    let commentDisplayColumns = [];
    let commentEditableColumns = [];
    ...
    if (tab.commentsEnabled) {
        try {
            ...
            if (comments.length > 0) {
                const allCommentColumns = Object.keys(comments[0]);
                
                // Fixed columns that should NOT be editable
                const FIXED_COLUMNS = ['PRIMARY_KEY', 'USERNAME', 'UPDATED TIME'];
                
                // Display columns: ALL except PRIMARY_KEY
                commentDisplayColumns = allCommentColumns.filter(col => col !== 'PRIMARY_KEY');
                
                // Editable columns: ALL except fixed columns
                commentEditableColumns = allCommentColumns.filter(col => !FIXED_COLUMNS.includes(col));
                
                console.log('Comment columns loaded:', {
                    all: allCommentColumns,
                    display: commentDisplayColumns,
                    editable: commentEditableColumns
                });
            }
        } catch (error) {
            console.warn('Comment file not found or error:', error);
            // FIX: Set empty arrays so button is disabled
            commentDisplayColumns = [];
            commentEditableColumns = [];
        }
    }
    ...
    STATE.allData[tabName] = {
        ...
        filteredData: [...data],
        comments: comments,
        commentDisplayColumns: commentDisplayColumns,
        commentEditableColumns: commentEditableColumns
    };
}

// FIX #3: CLIENT-SIDE SORTING
...
// FIX #5: TABLE RENDERING - Preserve all column values
function renderTable() {
    ...
    const commentDisplayColumns = tabData.commentDisplayColumns || [];
    ...
    if (tab.columnOrder && tab.columnOrder.length > 0) {
        // Add specified columns first (only if they exist in data)
    ...
    const allColumns = tab.commentsEnabled && commentDisplayColumns.length > 0 
        ? [...commentDisplayColumns, ...orderedDataColumns] 
        : orderedDataColumns;
    ...
    thead.innerHTML = `
        <tr>
            ...
            ${allColumns.map((col, idx) => {
                const isCommentColumn = commentDisplayColumns.includes(col);
                const sortClass = STATE.sortColumn === col 
                    ? (STATE.sortDirection === 'asc' ? 'sort-asc' : 'sort-desc')
                    : 'sortable';
                // Only make data columns sortable, not comment columns
                const sortable = !isCommentColumn;
    ...
    document.querySelectorAll('.ws-data-table th.sortable, .ws-data-table th.sort-asc, .ws-data-table th.sort-desc').forEach(th => {
    ...
    pageData.forEach((row, index) => {
        ...
        // Render comment columns
        if (tab.commentsEnabled && commentDisplayColumns.length > 0) {
            commentDisplayColumns.forEach(col => {
        ...
        // Render data columns (with proper ordering)
        orderedDataColumns.forEach(col => {
            const td = document.createElement('td');
            const value = row[col];
...
function applyFiltersFromModal() {
    ...
    document.querySelectorAll('.filter-select').forEach(select => {
        ...
        // Use CSS.escape for special characters
        const escapedColumn = CSS.escape(column);
...
function updateButtonVisibility() {
    ...
    // Comments button - FIX: Only show if comments enabled AND columns exist
    const tabData = STATE.allData[STATE.currentTab];
    const commentsBtn = document.getElementById('addCommentsButton');
    if (tab.commentsEnabled && tabData && tabData.commentEditableColumns && tabData.commentEditableColumns.length > 0) {
...
// FIX #6: SUMMARY - Negative sum in red
function updateSummary() {
    ...
    // FIX #6: Show negative sum in red
    if (sum < 0) {
...
// FIX #4: COMMENTS - Dynamic editable columns only
function openCommentsModal() {
    ...
    if (!tabData.commentEditableColumns || tabData.commentEditableColumns.length === 0) {
        showToast('Comment file not found or no editable columns', 'error');
        return;
    }
    ...
    // Show ONLY editable columns (exclude PRIMARY_KEY, USERNAME, UPDATED TIME)
    tabData.commentEditableColumns.forEach(col => {
...
async function saveComments() {
    try {
        ...
        // Collect ALL comment fields dynamically
        const commentData = {};
        ...
        selectedData.forEach(row => {
            ...
            if (commentIndex >= 0) {
                // Update existing comment - preserve all existing columns
                tabData.comments[commentIndex] = { ...tabData.comments[commentIndex], ...fullComment };
            } else {
...
// FIX #1: EDIT ROW - Preserve ALL row data
...
async function saveEditedRow() {
    try {
        ...
        // FIX #1: Start with ALL original row data
        const newRow = { ...oldRow };
        ...
        // Update in rawData using primary key matching
        const primaryKeyValue = oldRow[CONFIG.primaryKey];
        const rawDataIndex = tabData.rawData.findIndex(r => r[CONFIG.primaryKey] === primaryKeyValue);
...
console.log('Worksheet Tool v4 (All Fixes Applied) loaded!');
