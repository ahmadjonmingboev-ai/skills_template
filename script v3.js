...
let categoryCodeColumn = null; // Track the category code column name
...
async function loadData() {
    try {
        ...
        if (mappingData.length > 0) {
            ...
            // Find the category code column
            categoryCodeColumn = tableColumns.find(col => col.includes('CATEGORY')) || 'CATEGORY_CODE';
...
function generateFormFields() {
    ...
    for (let i = 0; i < tableColumns.length; i += fieldsPerRow) {
        ...
        for (let j = i; j < Math.min(i + fieldsPerRow, tableColumns.length); j++) {
            ...
            // Check if this is the category code field
            const isCategoryField = column === categoryCodeColumn;
            ...
            if (column === 'TLM_INSTANCE' || column === 'INSTANCE') {
                ...
                const placeholder = isCategoryField ? 
                    `${displayName} (comma-separated for multiple)` : 
                    displayName;
...
function openAddModal() {
    ...
    document.getElementById('categorySplitNote').style.display = 'block';
...
function openEditModal(index) {
    ...
    document.getElementById('categorySplitNote').style.display = 'none';
...
async function saveRecord() {
    ...
    tableColumns.forEach(column => {
        ...
        const field = document.getElementById(fieldId);
        if (field) {
            if (column === categoryCodeColumn && !isEdit) {
                // For category codes in add mode, split by comma
                const codes = field.value.split(',').map(code => code.trim()).filter(code => code);
                categoryCodes = codes;
            } else {
                baseRecord[column] = field.value;
            }
        }
    });
    ...
    try {
        ...
        if (isEdit) {
            ...
            const oldRecord = filteredData[parseInt(index)];
            ...
            if (actualIndex !== -1) {
                mappingData[actualIndex] = baseRecord;
            }

            // Add to history
            await addToHistory('UPDATE', baseRecord, oldRecord);
        } else {
            // Add mode - potentially multiple records
            if (categoryCodes.length > 0) {
                // Multiple category codes - create a record for each
                const newRecords = [];
                categoryCodes.forEach(code => {
                    const newRecord = { ...baseRecord };
                    newRecord[categoryCodeColumn] = code;
                    mappingData.push(newRecord);
                    newRecords.push(newRecord);
                });

                // Add to history for bulk insert
                await addToHistory(
                    'BULK_CREATE',
                    { count: categoryCodes.length, codes: categoryCodes.join(', ') },
                    null
                );
                
                showNotification(
                    'Records Added',
                    `${categoryCodes.length} records have been added successfully.`,
                    'success'
                );
            } else {
                // Single record
                mappingData.push(baseRecord);
                await addToHistory('CREATE', baseRecord, null);
...
async function addToHistory(action, newRecord, oldRecord) {
    const historyEntry = {
        ...
        User: currentUser,
        Action: action,
        RowID: action === 'BULK_CREATE' ? `Bulk: ${newRecord.count} records` : Object.values(newRecord).slice(0, 3).join('-'),
            ...
            row.innerHTML = `
                ...
                <td>
                    <span class="badge ${
                        ...
                        entry.Action === 'BULK_CREATE' ? 'badge-success' :
...
function generateTeamString() {
    ...
    // Generate simple team string
    if (branchKeys.size === 0) {
        showNotification('No Data', 'No branch keys found for the selected criteria.', 'warning');
        return;
    }

    let tableauString = 'IF ';
    const branchArray = Array.from(branchKeys);
    ...
    showNotification('Team String Generated', 'Team grouping string generated successfully.', 'success');
}
